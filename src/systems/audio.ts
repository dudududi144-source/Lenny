...
export function worldTimbre(){const types=['sine','triangle','square','sawtooth'];
 return {type:types[((state.world%4)+4)%4], root:261.6*(1+(((state.world%5)+5)%5)*0.12)};}
function note(f:number,dur:number,vol:number,type:OscillatorType='sine'){if(!ctx||!master)return;
...
export function startAudio(){try{
 ...
 setInterval(()=>{if(!ctx||!master)return;const L=audioLayers();const tim=worldTimbre();
