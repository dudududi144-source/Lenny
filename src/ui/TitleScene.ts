...
/* Title v2 — clean entry, no window globals */
export class TitleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    if (q === 'world') { this.scene.start('world'); return; }
    if (q === 'game') { this.scene.start('world'); return; }
    ...
    this.add.text(w / 2, h * 0.2, 'LENY', {
      fontFamily: 'Heebo', fontSize: '56px', color: '#FFD76A'
    }).setOrigin(0.5);

    this.add.text(w / 2, h * 0.3, 'גַּן שֶׁל אוֹרוֹת', {
      fontFamily: 'Heebo', fontSize: '24px', color: '#FFF6EC'
    }).setOrigin(0.5);

    this.add.text(w / 2, this.btn.y + 28, t('cta'), {
      fontFamily: 'Heebo', fontSize: '24px', color: '#2a2140'
    }).setOrigin(0.5);

    this.input.on('pointerdown', (p: Phaser.Input.Pointer) => {
      if (p.x > this.btn.x && p.x < this.btn.x + this.btn.w &&
          p.y > this.btn.y && p.y < this.btn.y + this.btn.h) {
  ...
  private render(time: number): void {
    ...
    const emotion = stateManager.getEmotion();

    drawAurora(this.bg, w, h, t, lights);
    drawDiorama(this.dio, w, h, t, w / 2, lights);
    drawMascot(this.mg, w / 2, h * 0.62, 1.2, t, emotion, 0, true);

    this.fg.clear();
    this.fg.fillStyle(0xffd76a, 1);
    this.fg.fillRoundedRect(this.btn.x, this.btn.y, this.btn.w, this.btn.h, 28);
  }
}
