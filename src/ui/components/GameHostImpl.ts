...
/**
 * The game screen implementation — LAZY (audit 9-c): this module (Pixi,
 * every scene, the whole engine) is a dynamic chunk fetched on the first
 * open only. The sync GameHost proxy owns `root` and hands it in here.
 *
 ...
 * scenes in and out.
 */
export function createGameHostImpl(callbacks: GameHostCallbacks, root: HTMLElement): GameHostHandle {
  ...
  root.append(stage, hud.root, shelf.root);
